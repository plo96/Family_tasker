...
def get_new_task_dict(name: str = f'{fake.text(MAX_TASK_NAME_LENGTH)}',
				 description: str = f'{fake.text(MAX_TASK_DESCRIPTION_LENGTH)}',
				 price: int = randint(MIN_TASK_PRICE, MAX_TASK_PRICE)) -> dict:
...
@pytest.fixture(scope='session', params=[_ for _ in range(NUM_TESTS)])
def new_task(request) -> TaskCreate:
	return TaskCreate(**get_new_task_dict())


@pytest.fixture(scope='session', params=[i for i in range(NUM_TESTS)])
...
@pytest.fixture(scope='session')
async def some_data_added() -> None:
